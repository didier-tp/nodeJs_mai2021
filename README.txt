tpNodeJs
--------
Didier Defrance / Asit Conseils
---------
https://github.com/didier-tp/nodeJs_mai2021

=============
Doc/exemple sur EJS: 
https://www.digitalocean.com/community/tutorials/how-to-use-ejs-to-template-your-node-application-fr
https://ejs.co/
Avantage de EJS (très semblable au vrai standard HTML, syntaxe claire).
==========
Doc/exemple sur Jade:
https://www.tutorialsteacher.com/nodejs/jade-template-engine
https://jade-lang.com/
Avantage de jade : syntaxe très compacte.
Inconvients: de moins en moins utilisé car copier/coller moins facile depuis exemples html standard

Remarque importante:
le projet open-source jade(npm) à été renommé "pug" car jade est une marque déposée (avec nom réservé)
https://www.npmjs.com/package/pug
https://expressjs.com/fr/guide/using-template-engines.html
https://www.sitepoint.com/a-beginners-guide-to-pug/
https://pugjs.org/api/getting-started.html

 
========
https://www.coursaline.com/tutoriels/developpement-web/creer-exemple-simple-popup

===========
intéressant (à néanmoins un peu adapter):
  JWT avec passportjs :
  https://www.digitalocean.com/community/tutorials/api-authentication-with-json-web-tokensjwt-and-passport

  HTTPS avec nodeJs:
  https://slproweb.com/products/Win32OpenSSL.html
  https://blog.goovy.io/running-a-nodejs-server-with-https/

  https://www.npmjs.com/package/node-windows (pour lancer nodeJs comme un service windows)